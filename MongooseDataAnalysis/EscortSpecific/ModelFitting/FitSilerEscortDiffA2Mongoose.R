...
pdf("outputs/SilerEscortDiffA2.pdf")

###########################################################
##                                                      ###
##   Now fit Siler model with escort differences on a2  ###
##                                                      ###
###########################################################
...
code <- nimbleCode({
  ...
  for (i in 1:nind) {
    ...
    tD[i] ~ dsilerNim(a1, a2[escort[i] + 1], b1, b2, c1)
  }
...
initFn <- function(cint, censored, escort) {
  ...
  optFn <- function(pars, t, escort) {
    ...
    llL <- sum(dSiler(t[escort == 0], a1 = pars[1], a2 = pars[2], b1 = pars[4], b2 = pars[5], c1 = pars[6], log = TRUE))
    llH <- sum(dSiler(t[escort == 1], a1 = pars[1], a2 = pars[3], b1 = pars[4], b2 = pars[5], c1 = pars[6], log = TRUE))
    llL + llH
  }
  ...
  ll <- sum(dSiler(tD[escort == 0], a1 = pars[1], a2 = pars[2], b1 = pars[4], b2 = pars[5], c1 = pars[6], log = TRUE))
  ll <- ll + sum(dSiler(tD[escort == 1], a1 = pars[1], a2 = pars[3], b1 = pars[4], b2 = pars[5], c1 = pars[6], log = TRUE))
...
## save MCMC
saveRDS(samples, "outputs/post_sedA2.rds")
...
pred_plot <- ggplot(pred_escort, aes(x = time)) +
  ...
  ggtitle("Comparison Survival Curves", subtitle = "KM plots of actual data against predicted escort specific differences in a2")
pred_plot

## pairs plot
samples <- as.matrix(samples)
colnames(samples) <- c("a1", "a2l", "a2h", "b1", "b2", "c1")
...
log.like <- function(zL, zU, censored, escort, pars) {
  ...
  ## interval-censored
  llI_l <- log(pSiler(zU[censored == 1 & escort == 0], pars[1], pars[2], pars[4], pars[5], pars[6]) - pSiler(zL[censored == 1 & escort == 0], pars[1], pars[2], pars[4], pars[5], pars[6]))
  llI_h <- log(pSiler(zU[censored == 1 & escort == 1], pars[1], pars[3], pars[4], pars[5], pars[6]) - pSiler(zL[censored == 1 & escort == 1], pars[1], pars[3], pars[4], pars[5], pars[6]))
  
  ## right-censored
  llR_l <- pSiler(zL[censored == 2 & escort == 0], pars[1], pars[2], pars[4], pars[5], pars[6], log = TRUE, lower.tail = FALSE)
  llR_h <- pSiler(zL[censored == 2 & escort == 1], pars[1], pars[3], pars[4], pars[5], pars[6], log = TRUE, lower.tail = FALSE)
...
saveRDS(logimpweight, "outputs/logimpweight_sedA2.rds")
...
## trace plots look like they haven't quite converged
## so run more IS samples
...
## calculate log-likelihoods in parallel
logimpweight1 <- apply(props, 1, list)
logimpweight1 <- purrr::map(logimpweight1, 1)
logimpweight1 <- mclapply(logimpweight1,
    function(pars, zL, zU, censored, escort) {
        log.like(zL, zU, censored, escort, pars)
    }, zL = zL, zU = zU, censored = censored, escort = escort, mc.cores = 24)
logimpweight1 <- reduce(logimpweight1, c)

## priors
logimpweight1 <- logimpweight1 + dexp(props[, 1], 1, log = TRUE) + 
  dexp(props[, 2], 1, log = TRUE) + dexp(props[, 3], 1, log = TRUE) +
  dexp(props[, 4], 1, log = TRUE) + dexp(props[, 5], 1, log = TRUE) +
  dexp(props[, 6], 1, log = TRUE)

## importance distributions
logimpweight1 <- logimpweight1 - 
  log(0.95 * dens(mod$modelName, props, FALSE, mod$parameters) + 0.05 * exp(dexp(props[, 1], 1, log = TRUE) + dexp(props[, 2], 1, log = TRUE) +
    dexp(props[, 3], 1, log = TRUE) + dexp(props[, 4], 1, log = TRUE) +
    dexp(props[, 5], 1, log = TRUE) + dexp(props[, 6], 1, log = TRUE)))
logimpweight <- c(logimpweight, logimpweight1)
saveRDS(logimpweight, "outputs/logimpweight_sedA2.rds")
