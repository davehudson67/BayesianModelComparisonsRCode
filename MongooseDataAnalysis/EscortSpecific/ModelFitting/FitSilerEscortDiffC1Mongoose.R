...
pdf("outputs/SilerEscortDiffC1.pdf")

###########################################################
##                                                      ###
##   Now fit Siler model with escort differences on c1  ###
##                                                      ###
###########################################################
...
code <- nimbleCode({
  ...
  for (i in 1:nind) {
    ...
    tD[i] ~ dsilerNim(a1, a2, b1, b2, c1[escort[i] + 1])
  }
...
initFn <- function(cint, censored, escort) {
  ...
  optFn <- function(pars, t, escort) {
    ...
    llL <- sum(dSiler(t[escort == 0], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[5], log = TRUE))
    llH <- sum(dSiler(t[escort == 1], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[6], log = TRUE))
    llL + llH
  }
  ...
  ll <- sum(dSiler(tD[escort == 0], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[5], log = TRUE))
  ll <- ll + sum(dSiler(tD[escort == 1], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[6], log = TRUE))
...
## save MCMC
saveRDS(samples, "outputs/post_sedc1.rds")
...
pred_plot <- ggplot(pred_escort, aes(x = time)) +
  ...
  ggtitle("Comparison Survival Curves", subtitle = "KM plots of actual data against predicted escort specific differences in c1")
pred_plot

## pairs plot
samples <- as.matrix(samples)
colnames(samples) <- c("a1", "a2", "b1", "b2", "c1l", "c1h")
...
log.like <- function(zL, zU, censored, escort, pars) {
  ...
  ## interval-censored
  llI_l <- log(pSiler(zU[censored == 1 & escort == 0], pars[1], pars[2], pars[3], pars[4], pars[5]) - pSiler(zL[censored == 1 & escort == 0], pars[1], pars[2], pars[3], pars[4], pars[5]))
  llI_h <- log(pSiler(zU[censored == 1 & escort == 1], pars[1], pars[2], pars[3], pars[4], pars[6]) - pSiler(zL[censored == 1 & escort == 1], pars[1], pars[2], pars[3], pars[4], pars[6]))
  
  ## right-censored
  llR_l <- pSiler(zL[censored == 2 & escort == 0], pars[1], pars[2], pars[3], pars[4], pars[5], log = TRUE, lower.tail = FALSE)
  llR_h <- pSiler(zL[censored == 2 & escort == 1], pars[1], pars[2], pars[3], pars[4], pars[6], log = TRUE, lower.tail = FALSE)
...
saveRDS(logimpweight, "outputs/logimpweight_sedC1.rds")
