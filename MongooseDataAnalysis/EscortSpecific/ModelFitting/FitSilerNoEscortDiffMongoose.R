...
pdf("outputs/SilerNoEscortDiff.pdf")

###########################################################
##                                                      ###
##   Now fit Siler model with no escort differences     ###
##                                                      ###
###########################################################
...
## save MCMC
saveRDS(samples, "outputs/post_sned.rds")
...
## plot ignoring escort differences
pred_plot <- km_plot[[1]] +
  geom_line(aes(x = t, y = Median), data = preds) +
  geom_ribbon(aes(x = t, y = Median, ymin = LCI, ymax = UCI), data = preds, alpha = 0.2)
pred_plot
...
pred_plot <- ggplot(pred_escort, aes(x = time)) +
  ...
  ggtitle("Comparison Survival Curves", subtitle = "KM plots of actual data against predicted no escorting differences")
pred_plot

## pairs plot
samples <- as.matrix(samples)
colnames(samples) <- c("a1", "a2", "b1", "b2", "c1")
...
log.like <- function(zL, zU, censored, pars) {
  ## calculate log-likelihoods
  ## interval-censored
  llI <- log(pSiler(zU[censored == 1], pars[1], pars[2], pars[3], pars[4], pars[5]) - pSiler(zL[censored == 1], pars[1], pars[2], pars[3], pars[4], pars[5]))

  ## right-censored
  llR <- pSiler(zL[censored == 2], pars[1], pars[2], pars[3], pars[4], pars[5], log = TRUE, lower.tail = FALSE)
...
saveRDS(logimpweight, "outputs/logimpweight_sned.rds")
