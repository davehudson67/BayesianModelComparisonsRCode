...
## load data
mong <- readRDS("../Data/mong_escortClean.rds")
...
## set up escort status
mong$escort_cat <- as.factor(mong$escort_cat)
escort <- as.numeric(mong$escort_cat) - 1

g <- length(levels(mong$escort_cat)) 

## produce K-M plot (assuming that interval-censored individuals
## die at the end time of the interval)
mong_km_dat <- select(mong, tD = death_day, tL = last_seen, tB = birth_day, cens = censored, escort_cat = escort_cat) %>%
...
## plot sex-specific KM
temp_fit <- surv_fit(Surv(tD, cens) ~ escort_cat, data = mong_km_dat)
kms_plot <- ggsurvplot(temp_fit, data = mong_km_dat, conf.int = TRUE)
kms_plot

## set and sample some random seeds for different models
set.seed(42)
seeds <- round(runif(50, 0, 100000000))
