...
## set up sex data (male = 1, female = 2, pup(this is where sex is unknown) = NA)
sex <- as.numeric(as.factor(mong$sex)) - 1
sex[sex == 2] <- NA

g <- length(levels(as.factor(mong$sex))) - 1 #remove pup group
...
mong_km_dat <- select(mong, tD = death_day, tL = last_seen, tB = birth_day, cens = censored, sex = sex) %>%
