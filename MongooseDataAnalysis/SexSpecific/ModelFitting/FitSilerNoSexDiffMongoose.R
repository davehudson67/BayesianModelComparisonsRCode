...
pdf("outputs/SilerNoSexDiff.pdf")

###########################################################
##                                                      ###
##   Now fit Siler model with no sex differences        ###
##                                                      ###
###########################################################
...
## save MCMC
saveRDS(samples, "outputs/post_s.rds")

## predictive plot against data
t_pred <- seq(0, max(cint), length.out = 100)
preds <- as.matrix(samples)[1:2000, -1] %>%
...
pred_plot <- ggplot(pred_sex, aes(x = time)) +
  ...
  geom_line(aes(x = t, y = Median), data = preds, alpha = 0.7) +
  geom_line(aes(x = t, y = LCI), data = preds, linetype = "dashed", alpha = 0.7) +
  geom_line(aes(x = t, y = UCI), data = preds, linetype = "dashed", alpha = 0.7) +
  xlab("Time") + ylab("Survival") + labs(colour = "Sex", fill = "Sex") +
  ggtitle("Comparison Survival Curves", subtitle = "KM plots of actual data against predicted non sex specific Siler")
pred_plot

## pairs plot
samples <- as.matrix(samples)
colnames(samples) <- c("Pm", "a1", "a2", "b1", "b2", "c1")
...
props <- props[, -1]
colnames(props) <- c("Pm", "a1", "a2", "b1", "b2", "c1")

## take random samples from prior (to create defense mixture)
defense <- matrix(runif(nimp - nmix, 0, 1), ncol = 1)
defense <- cbind(defense, matrix(rexp(5 * (nimp - nmix), 1), ncol = 5))
colnames(defense) <- c("Pm", "a1", "a2", "b1", "b2", "c1")
...
log.like <- function(zL, zU, censored, sex, pars) {
  ...
  llI_m <- log(pSiler(zU[censored == 1 & sex == 1 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6]) - pSiler(zL[censored == 1 & sex == 1 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6])) + log(pars[1])
  ...
  llR_m <- pSiler(zL[censored == 2 & sex == 1 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6], log = TRUE, lower.tail = FALSE) + log(pars[1])
  ...
  llI_miss <- llI_miss + pars[1] * (pSiler(zU[censored == 1 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6]) - pSiler(zL[censored == 1 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6]))
  ...
  llR_miss <- llR_miss + pars[1] * pSiler(zL[censored == 2 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6], lower.tail = FALSE)
