...
pdf("outputs/SilerSexSpecificC1.pdf")

###########################################################
##                                                      ###
##   Now fit Siler model with sex differences on c1     ###
##                                                      ###
###########################################################
...
code <- nimbleCode({
  ...
  for (i in 1:nind) {
    ...
    tD[i] ~ dsilerNim(a1, a2, b1, b2, c1[sex[i] + 1])
    sex[i] ~ dbern(Pm)
  }
...
initFn <- function(cint, censored, sex) {
  ...
  optFn <- function(pars, t, sex) {
    ...
    llM <- sum(dSiler(t[sex == 0], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[5], log = TRUE))
    llF <- sum(dSiler(t[sex == 1], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[6], log = TRUE))
    llM + llF
  }
  ...
  ll <- sum(dSiler(tD[sexI == 0], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[5], log = TRUE))
  ll <- ll + sum(dSiler(tD[sexI == 1], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[6], log = TRUE))
...
## save MCMC
saveRDS(samples, "outputs/post_ssdc1.rds")
...
pred_plot <- ggplot(pred_sex, aes(x = time)) +
  ...
  ggtitle("Comparison Survival Curves", subtitle = "KM plots of actual data against predicted sex specific differences in c1")
pred_plot

## pairs plot
samples <- as.matrix(samples)
colnames(samples) <- c("Pm", "a1", "a2", "b1", "b2", "c1f", "c1m")
...
props <- props[, -1]
colnames(props) <- c("Pm", "a1", "a2", "b1", "b2", "c1f", "c1m")
...
colnames(defense) <- c("Pm", "a1", "a2", "b1", "b2", "c1f", "c1m")
...
log.like <- function(zL, zU, censored, sex, pars) {
  ...
  llI_f <- log(pSiler(zU[censored == 1 & sex == 0 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6]) - pSiler(zL[censored == 1 & sex == 0 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6])) + log(1 - pars[1])
  llI_m <- log(pSiler(zU[censored == 1 & sex == 1 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[7]) - pSiler(zL[censored == 1 & sex == 1 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[7])) + log(pars[1])
  
  ## right-censored, known sex
  llR_f <- pSiler(zL[censored == 2 & sex == 0 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6], log = TRUE, lower.tail = FALSE) + log(1 - pars[1])
  llR_m <- pSiler(zL[censored == 2 & sex == 1 & !is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[7], log = TRUE, lower.tail = FALSE) + log(pars[1])
  
  ## interval-censored unknown sex
  llI_miss <- (1 - pars[1]) * (pSiler(zU[censored == 1 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6]) - pSiler(zL[censored == 1 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6]))
  llI_miss <- llI_miss + pars[1] * (pSiler(zU[censored == 1 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[7]) - pSiler(zL[censored == 1 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[7]))
  llI_miss <- log(llI_miss)
  
  ## right-censored unknown sex
  llR_miss <- (1 - pars[1]) * pSiler(zL[censored == 2 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[6], lower.tail = FALSE)
  llR_miss <- llR_miss + pars[1] * pSiler(zL[censored == 2 & is.na(sex)], pars[2], pars[3], pars[4], pars[5], pars[7], lower.tail = FALSE)
...
saveRDS(logimpweight, "outputs/logimpweight_ssdC1.rds")
