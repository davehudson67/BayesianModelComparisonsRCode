...
initFn <- function(cint, censored) {
  ...
  optFn <- function(pars, t, censored) {
    ...
    ll <- sum(dSiler(t[censored == 1], a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[5], log = TRUE))
  }
  ...
  while(pars$convergence != 0 & k < 20) {
    ...
    pars <- optim(rexp(5, 10), optFn, t = tD, censored = censored, control = list(fnscale = -1))
    k <- k + 1
  }
  if(k == 20) {
  ...
  tD[censored == 2] <- rSiler(sum(censored == 2), a1 = pars[1], a2 = pars[2], b1 = pars[3], b2 = pars[4], c1 = pars[5], zL = cint[censored == 2, 2])
...
#config$addSampler(target = c("c1"), type = 'slice')
