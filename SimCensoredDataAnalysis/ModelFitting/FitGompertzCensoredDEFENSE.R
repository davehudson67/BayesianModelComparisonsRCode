...
initFn <- function(cint, censored) {
  ...
  optFn <- function(pars, t, censored) {
    ...
    ll <- sum(dGompertz(t[censored == 1], a = pars[1], b = pars[2], log = TRUE))
  }
  ...
  while(pars$convergence != 0 & k < 20) {
    ...
    pars <- optim(rexp(2, 100), optFn, t = tD, censored = censored, control = list(fnscale = -1))
    k <- k + 1
  }
  if(k == 20) {
  ...
  tD[censored == 2] <- rGompertz(sum(censored == 2), a = pars[1], b = pars[2], zL = cint[censored == 2, 2])
